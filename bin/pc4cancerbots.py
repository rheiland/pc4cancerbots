#
# pc4cancerbots.py - top-level script that defines the overall GUi: tabs, etc. 
#
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/pc4cancerbots")  # does Windows like this?
    else:
        # local cache
        try:
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 pc4cancerbots-r7 config.xml")   # "-r7" suffix??
        else:
...
tool_title = widgets.Label(r'\(\textbf{Cancer Biorobots}\)')
if nanoHUB_flag or hublib_flag:
